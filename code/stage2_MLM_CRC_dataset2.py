...
CONFIG = {
    ...
    "taxonomy": [
        ...
        "STP Fault (Spanning Tree Protocol)",
        "BFD Down (Bidirectional Forwarding Detection)",
        "LACP Flapping (Link Aggregation Control Protocol)",
        "OSPF Neighbor Flapping (Open Shortest Path First)",
    ],
    "label_aliases": {
        "Power Supply Fault": "Power Supply Fault",
        "Fan Fault": "Fan Fault",
        "Optics Module Fault": "Optics Module Fault",
        "Port Failure": "Port Failure",

        "CRC Error": "CRC Error (Cyclic Redundancy Check)",
        "CRC Error (Cyclic Redundancy Check)": (
            "CRC Error (Cyclic Redundancy Check)"
        ),

        "STP Fault": "STP Fault (Spanning Tree Protocol)",
        "STP Fault (Spanning Tree Protocol)": (
            "STP Fault (Spanning Tree Protocol)"
        ),

        "BFD Down": "BFD Down (Bidirectional Forwarding Detection)",
        "BFD Down (Bidirectional Forwarding Detection)": (
            "BFD Down (Bidirectional Forwarding Detection)"
        ),
    ...
    "log_extract_pattern": (
        r"The operation error log is:\s*(.*?)\s*categories:"
    ),
    "log_prefix_pattern": None,
    "event_delimiter": "##",
    "default_input": None,
    "default_output_dir": "output_crc_ZTE",
}
...
SYSTEM_PROMPT = (
    ...
    "Use only evidence from the target log and keep the CRC output compact."
)



def extract_log(user_content: str, config: dict) -> str:
    if not isinstance(user_content, str):
        raise TypeError("user_content must be a string.")

    match = re.search(
        config["log_extract_pattern"],
        user_content,
        flags=re.DOTALL | re.IGNORECASE,
    )
    if not match:
        raise ValueError(
            f"Cannot extract log content for dataset {config['display_name']}."
        )

    log = match.group(1).strip()
    prefix_pattern = config.get("log_prefix_pattern")
    if prefix_pattern:
        log = re.sub(
            prefix_pattern,
            "",
...
def add_evidence_ids(log: str, config: dict) -> str:
    delimiter = config["event_delimiter"]
    events = [x.strip() for x in log.split(delimiter) if x.strip()]
    if not events:
...
def canonicalize_label(label, config: dict):
    if label is None:
        return None
    ...
    # Also allow an exact taxonomy string embedded in a verbose response.
    for item in sorted(taxonomy, key=len, reverse=True):
        if item in label:
            return item
...
def extract_cloud_label(reasoning_v1: str, config: dict):
    ...
    return canonicalize_label(matches[-1].strip(), config)
...
def compact_cloud_reasoning(reasoning_v1: str, max_chars: int) -> str:
    ...
    <=0 keeps the complete remaining cloud reasoning.
    """
    if not isinstance(reasoning_v1, str):
        return ""
...
def build_turn2_prompt(turn1_analysis, taxonomy):
    fs_text = json.dumps(
        {
            "F": turn1_analysis["F"],
            "S": turn1_analysis["S"],
    ...
    return (
        "Turn 2: determine the final fault label L from the CRC representation "
        "produced in Turn 1.\n"
        f"CRC_ANALYSIS={fs_text}\n\n"
        f"Fault taxonomy Y:\n{taxonomy_text}\n\n"
        "Choose exactly one class best supported by F and S. "
        "Confidence gamma_L is your confidence in that final label. "
        'Reply only as "<id>|<confidence_percent>", for example 1|92.'
    )



def parse_turn2_label(text: str, taxonomy, config):
    """
    Parse arbitrary 1..N class IDs and normalize confidence to [0,1].
    Accepts e.g. 3|82, 3|82%, or 3|0.82.
    """
    if not isinstance(text, str):
    ...
    if m:
        class_id_int = int(m.group(1))
        if 1 <= class_id_int <= n_classes:
            ...
            if confidence is not None:
                label_id = str(class_id_int)
                return (
                    label_id,
                    taxonomy[class_id_int - 1],
                    confidence,
                )

    # Class id only.
    for match in re.finditer(r"\b(\d+)\b", cleaned):
        class_id_int = int(match.group(1))
        if 1 <= class_id_int <= n_classes:
            label_id = str(class_id_int)
            return (
                label_id,
    ...
    # Full/aliased label fallback.
    label = canonicalize_label(cleaned, config)
    if label is not None:
        return (
            str(taxonomy.index(label) + 1),
            label,
            0.0,
        )
...
def canonicalize_crc_result(
    ...
    # Preserve the dataset2/ZTE script behavior:
    # canonical downstream output prefers the upstream cloud label.
    if cloud_label in config["taxonomy"]:
        L["fault_type"] = cloud_label
    else:
        L["fault_type"] = canonicalize_label(
            original_fault_type,
            config,
        )
...
def run_two_turn_crc(
    ...
    cuda_device = torch.device(input_device)

    # Peak memory across the whole two-turn case.
    torch.cuda.synchronize(cuda_device)
    torch.cuda.reset_peak_memory_stats(cuda_device)
    baseline_allocated_bytes = torch.cuda.memory_allocated(cuda_device)
    ...
    turn1 = generate_one_turn(
        ...
        power_sampling_hz=power_sampling_hz,
    )

    parsed_turn1_analysis = parse_turn1_analysis(
        turn1["response"],
        annotated_window,
    )
    turn1_analysis_parsed = parsed_turn1_analysis is not None
    turn1_used_fallback = not turn1_analysis_parsed

    if parsed_turn1_analysis is None:
        ...
        turn1_analysis = {
            ...
            "γ_S": 0.5,
            "Explanation": (
                "Fallback CRC structure was used because the first-turn "
                "model output could not be parsed."
            ),
        }
    else:
        turn1_analysis = parsed_turn1_analysis

    turn2_prompt = build_turn2_prompt(
        turn1_analysis,
        taxonomy,
    )

    # True second dialogue turn: retain turn-1 history.
    turn2_messages = [
        {"role": "system", "content": SYSTEM_PROMPT},
        {"role": "user", "content": turn1_prompt},
    ...
    label_id, predicted_label, gamma_l = parse_turn2_label(
        turn2["response"],
        taxonomy,
        config,
    )
    label_parse_ok = predicted_label is not None
    label_used_fallback = False

    # No-fail label gate: prefer upstream cloud label if Turn 2 formatting fails.
    if predicted_label is None:
        label_used_fallback = True
        if cloud_label in taxonomy:
            predicted_label = cloud_label
            label_id = str(taxonomy.index(cloud_label) + 1)
            gamma_l = 0.5
        else:
            predicted_label = taxonomy[0]
            label_id = "1"
            gamma_l = 0.5
    ...
    case_wall_end = time.perf_counter()
    ...
    token_throughput = (
        total_generated_tokens / total_infer_time
        if total_infer_time > 0
        else None
    )
    avg_power = (
        total_energy_j / total_infer_time
        if total_infer_time > 0
        else None
    )
    ...
    return {
        ...
        "turn1_analysis_parsed": turn1_analysis_parsed,
        "turn1_used_fallback": turn1_used_fallback,
        "turn2_raw": turn2["response"],
        "predicted_label_id": label_id,
        "predicted_label": predicted_label,
        "label_parse_ok": label_parse_ok,
        "label_used_fallback": label_used_fallback,
        "crc_result": crc_result,

        "turn1_prompt_tokens": turn1["prompt_tokens"],
        "turn1_generated_tokens": turn1["generated_tokens"],
        "turn1_inference_time_s": turn1["inference_time_s"],
        "turn1_gpu_power_avg_w": turn1["gpu_power_avg_w"],
        "turn1_energy_consumption_j": turn1["energy_consumption_j"],

        "turn2_prompt_tokens": turn2["prompt_tokens"],
        "turn2_generated_tokens": turn2["generated_tokens"],
        "turn2_inference_time_s": turn2["inference_time_s"],
        "turn2_gpu_power_avg_w": turn2["gpu_power_avg_w"],
        "turn2_energy_consumption_j": turn2["energy_consumption_j"],

        "prompt_tokens_total_across_calls": (
            turn1["prompt_tokens"] + turn2["prompt_tokens"]
        ),
        ...
        "inference_time_s": float(total_infer_time),
        "case_wall_time_s": float(
            case_wall_end - case_wall_start
        ),
        ...
        "gpu_power_sampling_hz": float(power_sampling_hz),
        "energy_consumption_j": float(total_energy_j),
    }
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Dataset2/ZTE two-turn CRC benchmark with no parse-failure gate."
        )
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Input JSON path. If omitted, the dataset2/ZTE default is used "
            "when configured."
        ),
    )
    parser.add_argument(
        "--model-path",
        default="/home/shared/models/Mistral-7B-Instruct-v0.3",
    )
    parser.add_argument(
        "--output-dir",
        default=None,
        help="Output directory; dataset2/ZTE default is used when omitted.",
    )
    parser.add_argument(
        "--max-analysis-tokens",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="<=0 keeps the complete cloud reasoning after removing Label.",
    )
    parser.add_argument(
        "--power-sampling-hz",
        type=float,
...
def main():
    ...
    input_value = (
        ...
        else config.get("default_input")
    )
    if not input_value:
        raise ValueError(
            f"No default input path is configured for {config['display_name']}. "
            "Pass --input /path/to/data.json"
        )

    output_value = (
        args.output_dir
        if args.output_dir is not None
        else f"{config['default_output_dir']}_two_turn_no_fail"
    )
    ...
    jsonl_path = output_dir / "crc_results.jsonl"
    json_path = output_dir / "crc_results.json"
    summary_path = output_dir / "crc_summary.json"
    ...
    pending = [
        x for x in data
        if record_key(x) not in completed_keys
    ]

    print(f"Dataset: {config['display_name']}")
    print(f"Classes: {len(taxonomy)}")
    print(f"Event delimiter: {config['event_delimiter']!r}")
    print(
    ...
    print(
        "Two-turn no-fail CRC | "
        f"Turn1<={args.max_analysis_tokens} tokens | "
        f"Turn2<={args.max_label_tokens} tokens | "
        f"cloud_reasoning_max_chars={args.cloud_reasoning_max_chars}"
    )

    if not pending:
        jsonl_to_latest_json(jsonl_path, json_path)
        print("Nothing to run.")
        return

    tokenizer, model, input_device = load_model(args.model_path)
    nvml_handle = init_nvml_handle_for_visible_gpu()
    ...
    try:
        for record in tqdm(
            pending,
            desc=f"CRC-2turn-{config['display_name']}",
        ):
            ...
            try:
                ...
                annotated_window = add_evidence_ids(
                    raw_log,
                    config,
                )

                reasoning_v1 = record["assistant_content"]
                cloud_label = extract_cloud_label(
                    reasoning_v1,
                    config,
                )
                compact_reasoning = compact_cloud_reasoning(
                    reasoning_v1,
                ...
                infer = run_two_turn_crc(
                    ...
                    input_device=input_device,
                    caseid=caseid,
                    annotated_window=annotated_window,
                    ...
                    nvml_handle=nvml_handle,
                    power_sampling_hz=args.power_sampling_hz,
                )

                counters["turn1_parsed"] += int(
                    infer["turn1_analysis_parsed"]
                )
                counters["turn1_fallback"] += int(
                    infer["turn1_used_fallback"]
                )
                counters["label_parsed"] += int(
                    infer["label_parse_ok"]
                )
                counters["label_fallback"] += int(
                    infer["label_used_fallback"]
                )

                crc_result = infer["crc_result"]

                raw_ok, raw_errors = validate_crc_structure(
                    crc_result,
                    annotated_window,
                    taxonomy,
                )

                canonical = canonicalize_crc_result(
                    crc_result,
                    annotated_window,
                    cloud_label=cloud_label,
                    config=config,
                )
                canonical_ok, canonical_errors = validate_crc_structure(
                    canonical,
                    annotated_window,
                    taxonomy,
                )

                counters["raw_valid"] += int(raw_ok)
                counters["canonical_valid"] += int(canonical_ok)
                ...
                output_record = {
                    ...
                    "cloud_label": cloud_label,
                    "compact_cloud_reasoning": compact_reasoning,
                    "taxonomy": taxonomy,

                    "conversation_turns": 2,
                    "turn1_raw": infer["turn1_raw"],
                    "turn1_analysis_parsed": infer["turn1_analysis_parsed"],
                    "turn1_used_fallback": infer["turn1_used_fallback"],
                    "turn2_raw": infer["turn2_raw"],
                    "predicted_label_id": infer["predicted_label_id"],
                    "predicted_label": infer["predicted_label"],
                    "label_parse_ok": infer["label_parse_ok"],
                    "label_used_fallback": infer["label_used_fallback"],

                    "crc_result": crc_result,
                    "canonical_crc_result": canonical,
                    "raw_structure_ok": raw_ok,
                    "raw_structure_errors": raw_errors,
                    "canonical_structure_ok": canonical_ok,
                    "canonical_structure_errors": canonical_errors,

                    "turn1_prompt_tokens": infer["turn1_prompt_tokens"],
                    "turn1_generated_tokens": infer["turn1_generated_tokens"],
                    "turn1_inference_time_s": infer["turn1_inference_time_s"],
                    "turn1_gpu_power_avg_w": infer["turn1_gpu_power_avg_w"],
                    "turn1_energy_consumption_j": infer["turn1_energy_consumption_j"],

                    "turn2_prompt_tokens": infer["turn2_prompt_tokens"],
                    "turn2_generated_tokens": infer["turn2_generated_tokens"],
                    "turn2_inference_time_s": infer["turn2_inference_time_s"],
                    "turn2_gpu_power_avg_w": infer["turn2_gpu_power_avg_w"],
                    "turn2_energy_consumption_j": infer["turn2_energy_consumption_j"],

                    "prompt_tokens_total_across_calls": infer[
                        "prompt_tokens_total_across_calls"
                    ],
                    "generated_tokens": infer["generated_tokens"],
                    "token_throughput_tokens_per_s": infer[
                        "token_throughput_tokens_per_s"
                    ],
                    "inference_time_s": infer["inference_time_s"],
                    "case_wall_time_s": infer["case_wall_time_s"],

                    "gpu_memory_baseline_allocated_bytes": infer[
                        "gpu_memory_baseline_allocated_bytes"
                    ],
                    "gpu_memory_baseline_allocated_gib": infer[
                        "gpu_memory_baseline_allocated_gib"
                    ],
                    "gpu_memory_peak_allocated_bytes": infer[
                        "gpu_memory_peak_allocated_bytes"
                    ],
                    "gpu_memory_peak_allocated_gib": infer[
                        "gpu_memory_peak_allocated_gib"
                    ],
                    "gpu_memory_peak_increment_bytes": infer[
                        "gpu_memory_peak_increment_bytes"
                    ],
                    "gpu_memory_peak_increment_gib": infer[
                        "gpu_memory_peak_increment_gib"
                    ],
                    "gpu_power_avg_w": infer["gpu_power_avg_w"],
                    "gpu_power_sampling_hz": infer["gpu_power_sampling_hz"],
                    "energy_consumption_j": infer["energy_consumption_j"],

                    "model_path": args.model_path,
                    "max_analysis_tokens": args.max_analysis_tokens,
                    "max_label_tokens": args.max_label_tokens,
                    "cloud_reasoning_max_chars": args.cloud_reasoning_max_chars,
                    "do_sample": False,
                }

                if (
                    infer["turn1_used_fallback"]
                    or infer["label_used_fallback"]
                    or not raw_ok
                    or not canonical_ok
                ):
                    output_record["warning"] = (
                        "Inference completed; no-fail fallback/diagnostic issue: "
                        f"turn1_fallback={infer['turn1_used_fallback']}, "
                        f"label_fallback={infer['label_used_fallback']}, "
                        f"raw_valid={raw_ok}, canonical_valid={canonical_ok}."
                    )

                append_jsonl(output_record, jsonl_path)

                # No-fail gate: all completed two-turn model inferences are successful.
                counters["successful"] += 1

                total_time += infer["inference_time_s"]
                total_wall_time += infer["case_wall_time_s"]
                total_turn1_time += infer["turn1_inference_time_s"]
                total_turn2_time += infer["turn2_inference_time_s"]

                total_generated_tokens += infer["generated_tokens"]
                total_turn1_tokens += infer["turn1_generated_tokens"]
                total_turn2_tokens += infer["turn2_generated_tokens"]

                total_energy_j += infer["energy_consumption_j"]
                case_avg_powers_w.append(infer["gpu_power_avg_w"])
                case_peak_memory_gib.append(
                    infer["gpu_memory_peak_allocated_gib"]
                )
                case_peak_memory_increment_gib.append(
                    infer["gpu_memory_peak_increment_gib"]
                )
                ...
                tqdm.write(
                    ...
                    f"fallback={infer['turn1_used_fallback'] or infer['label_used_fallback']} | "
                ...
                tqdm.write(
                    f"[ERROR] caseid={caseid}, part={part}: {exc}"
                )

                if is_fatal_cuda_error(exc):
                    jsonl_to_latest_json(jsonl_path, json_path)
                    raise RuntimeError(
                        "Fatal CUDA error detected. Restart the process before continuing."
                    ) from exc
    ...
    summary = {
        ...
        "taxonomy": taxonomy,
        "event_delimiter": config["event_delimiter"],
        ...
        "records_requested_this_run": len(pending),
        "successful_inference_this_run": n,
        "failed_this_run": counters["failed"],
        "turn1_parsed_this_run": counters["turn1_parsed"],
        "turn1_fallback_this_run": counters["turn1_fallback"],
        "label_parsed_this_run": counters["label_parsed"],
        "label_fallback_this_run": counters["label_fallback"],
        "raw_structure_valid_this_run": counters["raw_valid"],
        "canonical_structure_valid_this_run": counters["canonical_valid"],

        "total_inference_time_s_this_run": total_time,
        "mean_inference_time_per_case_s_this_run": (
            total_time / n if n else None
        ),
        "mean_turn1_inference_time_s_this_run": (
            total_turn1_time / n if n else None
        ),
        "mean_turn2_inference_time_s_this_run": (
            total_turn2_time / n if n else None
        ),
        "mean_case_wall_time_s_this_run": (
            total_wall_time / n if n else None
        ),

        "total_generated_tokens_this_run": total_generated_tokens,
        "mean_generated_tokens_per_case_this_run": (
            total_generated_tokens / n if n else None
        ),
        "mean_turn1_generated_tokens_this_run": (
            total_turn1_tokens / n if n else None
        ),
        "mean_turn2_generated_tokens_this_run": (
            total_turn2_tokens / n if n else None
        ),
        ...
        "mean_gpu_memory_peak_allocated_gib_this_run": (
            sum(case_peak_memory_gib) / len(case_peak_memory_gib)
            if case_peak_memory_gib
            else None
        ),
        "max_gpu_memory_peak_increment_gib_this_run": (
            max(case_peak_memory_increment_gib)
            if case_peak_memory_increment_gib
            else None
        ),

        "power_sampling_hz": args.power_sampling_hz,
        "total_energy_consumption_j_this_run": total_energy_j,
        "mean_energy_consumption_j_per_case_this_run": (
            total_energy_j / n if n else None
        ),
        "mean_case_avg_gpu_power_w_this_run": (
            sum(case_avg_powers_w) / len(case_avg_powers_w)
        ...
        "note": (
            "Dataset2/ZTE two-turn CRC with no parse-failure gate. "
            "All cases that complete both model.generate() calls are counted as "
            "successful. Turn-1 parse failure uses a minimal F/S fallback; Turn-2 "
            "label parse failure prefers the upstream cloud label as fallback. "
            "Parsing/structure flags are diagnostics only. Runtime is the sum of "
            "the two model.generate() wall-clock intervals; energy is the sum of "
            "the two NVML-measured turn energies."
        ),
    }
